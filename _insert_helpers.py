...
def current_equity(px: float|None=None) -> float:
    try: return estimate_equity(px)
    except Exception: return estimate_equity()

def compute_regime(df_feat):
    try:
        adx = ADXIndicator(df_feat["high"], df_feat["low"], df_feat["close"], window=ADX_WINDOW).adx()
    ...
    atr_now = float(df_feat["atr"].iloc[-1])
...
def update_position_management(px, row, effective_tp):
    global open_position, bars_in_position, loss_streak
