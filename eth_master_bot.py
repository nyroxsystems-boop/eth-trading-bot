...
def main():
    ...
    # === v10 PURGE: Worker-only — scoring synced to v6 weights + OOS kill-gate fix ===
    # v9 zeroed old strategies but new ones were scored with old weights (roi*3, sharpe*2).
    # v10 zeros everything again so fresh strategies use correct v6 weights (roi*15, sharpe*5, PF).
    try:
        from db_adapter import get_db_connection, USE_POSTGRES
        if USE_POSTGRES:
            with get_db_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("SELECT value FROM kv_store WHERE key = 'worker_v10_scoring_sync'")
                if not cursor.fetchone():
                    cursor.execute("UPDATE learning_strategies SET score = 0 WHERE score > 0")
                    purged = cursor.rowcount
                    cursor.execute("""
                        INSERT INTO kv_store (key, value) VALUES ('worker_v10_scoring_sync', 'true')
                        ON CONFLICT (key) DO UPDATE SET value = 'true'
                    """)
                    if purged > 0:
                        log(f"🧹 WORKER v10 PURGE: Zeroed {purged} strategies (v6 scoring sync + OOS kill-gate)")
                    else:
                        log("✅ Worker v10: no strategies to purge")
    except Exception as e:
        log(f"⚠️ Worker v10 purge: {e}")
